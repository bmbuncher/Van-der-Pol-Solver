...
def VDP_solve(t_max = 5000.0, x0 = 0.0, y0 = 0.1, mu = 1.0):
    ts = np.linspace(0.0, t_max, int(t_max) * 1000)
    xs = odeint(VDP_deriv, [x0, y0], ts, args = (mu,))
    
    return ts, xs

def plot_phase_space(data_in = None, t_max = 5000.0, x0 = 0.0, y0 = 0.1, mu = 1.0, resolution = 30, line_color = 'k', quiv_cmap = cm.gnuplot, arrow_locs = None, arrowsize = 1.0, arrow_fc = None, arrow_ec = None, arrow_style = '->', arrow_lw = 1.0, figsize = (10, 10), cbar_params = None, norm_type = 'linear', filename = None):
    if data_in is not None:
        ts, xs = data_in
    else:
        ts, xs = VDP_solve(t_max, x0, y0, mu)
    ...
    Q = ax_phase.quiver(X, Y, U_norm, V_norm, C, units = 'xy', cmap = quiv_cmap)
    ...
    ax_phase.set_ylabel('$y$', fontsize = 20)
...
def animate_phase_space_setup(data_in = None, t_max = 5000.0, x0 = 0.0, y0 = 0.1, mu = 1.0, resolution = 30, quiv_cmap = cm.gnuplot, figsize = (10, 10), cbar_params = None, norm_type = 'linear'):
    if data_in is not None:
        ts, xs = data_in
    else:
        ts, xs = VDP_solve(t_max, x0, y0, mu)
